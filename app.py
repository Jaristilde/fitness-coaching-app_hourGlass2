# streamlit_app.py
# Hourglass Workout Program by Joane Aristilde — PDF-accurate Streamlit app
...
def find_exercise_video(key_slug):
    ensure_dirs()
    safe_key = re.sub(r'[^a-z0-9_]+', '_', key_slug.lower())
    candidates = [os.path.join(EXERCISE_VIDEOS_DIR, f) for f in os.listdir(EXERCISE_VIDEOS_DIR) if safe_key in f.lower()]
...
def warmup_item(): return {"name":"Booty/ Leg Activation","sets":"—","reps":"5 min","category":"Warm-up"}
def stretching_item(): return {"name":"Stretching","sets":"—","reps":"5 min","category":"Recovery"}

# Updated stairmaster workouts (replacing treadmill)
def stairmaster_L1(): return {"name":"Stairmaster Workout","sets":"—","reps":"30 min: fat loss levels 8-10","category":"Cardio"}
def stairmaster_L2(): return {"name":"Stairmaster Workout","sets":"—","reps":"30 min: fat loss levels 8-10","category":"Cardio"}
def stairmaster_booty_warmup(): return {"name":"Stairmaster Warm-up","sets":"—","reps":"20 min: levels 5-6 (booty day warm-up)","category":"Warm-up"}
...
ABDUCTORS={"name":"Abductors","sets":"(1 warm up set) + 3","reps":"10-12 reps; 12-15 reps (last set)","category":"Booty"}
BULGARIAN={"name":"Bulgarian Split Squats","sets":"1 warm up set + 3 (each side) + 1 AMRAP (no weight)","reps":"10-12 reps; 8 reps (last set)","category":"Legs"}
LEG_CURL={"name":"Leg Curl","sets":"(1 warm up set) + 3","reps":"10-12 reps; 12-15 reps (last set)","category":"Legs"}
LEG_PRESS={"name":"Leg Press","sets":"1 warm up set + 3","reps":"10-12 reps; 8-10 reps (last set)","category":"Legs"}
...
BOOTY_FINISHER_B={
    "name":"Leg Finisher (light weight) — Focus Sumo + Jumps","sets":"1 set each","reps":"Sumo Squats 15-20 reps, Squat Jumps 10-12 reps, Single Leg Hip Thrust 15-20 reps",
...
BOOTY_L1 = [stairmaster_booty_warmup(), warmup_item(), KICKBACKS, HIP_THRUST, HYPEREXT, RDLS, stairmaster_L1(), stretching_item()]
...
LEGS_BOOTY_L2 = [warmup_item(), HIP_THRUST, HYPEREXT, BULGARIAN, LEG_PRESS, LEG_CURL, stairmaster_L2(), stretching_item()]

# Light Shoulders & Back
LSB_L1 = [
    {"name":"Warm Up Mobility","sets":"—","reps":"5 min","category":"Warm-up"},
    {"name":"Lat Pulldown Wide Grip","sets":"1 warm up set + 3","reps":"10-12 reps; 8-10 reps (last set)","category":"Back"},
...
SH_ABS = [
    ...
    {"name":"Repeat abs & core 2x","sets":"—","reps":"Complete the circuit twice","category":"Core"},
]
...
def render_hero():
    ...
    if mode == "Cover Photo":
        with c1:
            ...
            st.image(cover, use_container_width=True, caption="Your Transformation Journey") if cover else st.info("Upload a cover photo on the right.")
        with c2:
            ...
            if up:
                ...
                st.success("Cover photo saved!"); st.image(dst, use_container_width=True)
            existing = _latest(MAIN_MEDIA_DIR, "cover_")
            if existing and st.button("Delete current cover photo"):
                os.remove(existing); st.success("Photo deleted!"); st.rerun()
    else:
        with c1:
            vid = _latest(MAIN_MEDIA_DIR, "intro_")
            st.video(vid); st.caption("Your intro video") if vid else st.info("Upload an intro video on the right.")
        with c2:
            ...
            if upv:
                ...
                st.success("Intro video saved!"); st.video(dst)
...
# Program logic
# -----------------------------
def current_day_name(): return date.today().strftime("%A")
def get_schedule(level): return PROGRAM_SPLIT[f"Level {level}"]
...
def get_exercises_for_day(level, day_name, picked_label=None):
    ...
    if label == "REST":
        return [{"name":"Rest Day","sets":"—","reps":"Active recovery, stretching, or light walk","category":"Recovery"}]
    if label.startswith("ABS/CORE ONLY"): return ABS_CORE_ONLY
    if label == "ABS/CORE & CARDIO":
        arr = ABS_CORE_CIRCUIT.copy(); arr.append(stairmaster_L1() if level==1 else stairmaster_L2()); return arr
    ...
    return [{"name":label,"sets":"—","reps":"","category":"General"}]

def weekly_schedule(level):
    st.markdown('<div class="weekly-box"><h4 style="text-align:center;color:#FF1493;margin:0;">YOUR WEEKLY WORKOUT PLAN</h4></div>', unsafe_allow_html=True)
    schedule = get_schedule(level); today = current_day_name()
    for day, label in schedule.items():
        cols = st.columns([2,4,2])
        with cols[0]: st.markdown(f"**{day}**")
        with cols[1]:
            if label == "REST":
                ...
                if st.button(label, key=f"wkbtn_{level}_{day}", use_container_width=True):
                    ...
                    st.rerun()
        with cols[2]: st.markdown("👈 TODAY" if day==today else "")
...
def render_exercise_card(exercise, idx, workout_label, workout_date):
    name = exercise.get("name",""); sets = exercise.get("sets","—"); reps = exercise.get("reps","—")
    category = exercise.get("category","General"); ex_key = exercise_key(workout_label, name, workout_date)
    total_sets = parse_set_count(sets); is_finisher_group = isinstance(exercise.get("submoves"), list)
    ...
    if is_finisher_group:
        ...
        total = len(exercise["submoves"]); fully_done = (completed==total and total>0)
    else:
        for s in range(1, total_sets+1):
            if f"{ex_key}_set_{s}" in st.session_state.completed_exercises: completed += 1
        total = total_sets; fully_done = (completed==total and total>0)
    ...
    with c1:
        if is_finisher_group:
            ...
            for i, sub in enumerate(exercise["submoves"], start=1):
                ...
                chk = st.checkbox(f"{sub['name']} — {sub.get('reps','')}", key=f"chk_{smk}", value=done)
            ...
            parts = [p.strip() for p in sets.split('+')]; counter = 1
            warm_noted = False
            for p in parts:
                low = p.lower()
                if 'warm' in low:
                ...
                if 'amrap' in low:
                    ...
                    chk = st.checkbox("AMRAP Set", key=f"chk_{sk}", value=(sk in st.session_state.completed_exercises))
                    if chk and sk not in st.session_state.completed_exercises: st.session_state.completed_exercises.append(sk); save_user_data()
                    if not chk and sk in st.session_state.completed_exercises: st.session_state.completed_exercises.remove(sk); save_user_data()
                    counter += 1
                    continue
                ...
                if nums:
                    for w in range(1, int(nums[0])+1):
                        sk = f"{ex_key}_set_{counter}"
                        chk = st.checkbox(f"Working Set {w}", key=f"chk_{sk}", value=(sk in st.session_state.completed_exercises))
                        if chk and sk not in st.session_state.completed_exercises: st.session_state.completed_exercises.append(sk); save_user_data()
                        if not chk and sk in st.session_state.completed_exercises: st.session_state.completed_exercises.remove(sk); save_user_data()
                        counter += 1
            if total > 0: st.info(f"Completed: {completed}/{total} sets")
            else:
                simp_key = f"{ex_key}_simple"
                chk = st.checkbox("Mark done", key=f"chk_{simp_key}", value=(simp_key in st.session_state.completed_exercises))
                if chk and simp_key not in st.session_state.completed_exercises: st.session_state.completed_exercises.append(simp_key); save_user_data()
                if not chk and simp_key in st.session_state.completed_exercises: st.session_state.completed_exercises.remove(simp_key); save_user_data()

    with c2:
        st.markdown('<div class="video-section"><b>📹 Demo Video (optional)</b></div>', unsafe_allow_html=True)
        existing = find_exercise_video(ex_key)
        if existing and os.path.exists(existing):
            st.video(existing)
            if st.button(f"Delete video for {name}", key=f"delvid_{ex_key}"):
                try: os.remove(existing); st.success("Video deleted."); st.rerun()
...
def render_workout():
    ...
    with cols[0]:
        if st.button("Level 1", use_container_width=True):
            st.session_state.selected_level=1; st.session_state.selected_workout=None; st.session_state.selected_workout_day=None; save_user_data(); st.rerun()
    with cols[1]:
        if st.button("Level 2", use_container_width=True):
            st.session_state.selected_level=2; st.session_state.selected_workout=None; st.session_state.selected_workout_day=None; save_user_data(); st.rerun()
    with cols[2]: st.info(f"**Current: Level {st.session_state.selected_level}**")
    ...
    if st.session_state.get('selected_workout'):
        ...
        st.subheader(f"**{the_day}: {workout_label}** (Level {level})"); workout_badge(workout_label)
        if st.button("🔄 Return to Today's Workout"):
            st.session_state.selected_workout=None; st.session_state.selected_workout_day=None; st.rerun()
    else:
        today = current_day_name()
        workout_label = get_schedule(level).get(today, "REST")
        st.subheader(f"**Today ({today}): {workout_label}** (Level {level})"); workout_badge(workout_label)

    workout_date = date.today().isoformat()
    exercises = get_exercises_for_day(level, st.session_state.get('selected_workout_day', current_day_name()), picked_label=workout_label)
    ...
    # Summary
    total_ex = len(exercises); total_sets_all=0; completed_sets_all=0; completed_exercises=0
    for ex in exercises:
        name = ex["name"]; ex_key = exercise_key(workout_label, name, workout_date)
        if "submoves" in ex:
            sub_total=len(ex["submoves"]); sub_done=0
            ...
            total_sets_all+=sub_total; completed_sets_all+=sub_done
            if sub_total>0 and sub_done==sub_total: completed_exercises+=1
        else:
            t = parse_set_count(ex.get("sets","—")); total_sets_all+=t; done=0
...
# -----------------------------
# Meal Plan Sample — Weekly plans + Daily Intake Guide + Education
# -----------------------------
WEEKLY_MEALS = {
    "Option A: Omnivore": {
...
CARB_CHOICES_67g = ["Apple ~2⅔ pcs (≈556 g)", "Sweet potato ~1½ serving (≈216 g)",
                    "Banana ~2⅔ pcs (≈319 g)", "Brown rice ~1 serving (≈204 g)",
                    "Oatmeal ~1 serving (≈97 g)", "Grapefruit 3 pcs (≈886 g)",
                    "Whole wheat bread ~1⅔ serving (≈118 g)"]
PRO_CHOICES_40g = ["Salmon ~4 servings (≈197 g)", "Chicken breast ~1⅔ serving (≈175 g)",
                   "Egg ~6½ servings (≈323 g)", "Beef rump ~1 serving (≈192 g)",
                   "Pork tenderloin ~1½ serving (≈288 g)", "Pea/soy protein scoop per label"]
FAT_CHOICES_16g = ["Almond ~1 serving (29 g)", "Olive oil ~3⅓ tsp (16 g)",
                   "Avocado ~1 piece (86 g)", "Cheese ~3½ servings (70 g)"]
...
def render_weekly_meal_plans():
    ...
    st.markdown("**Vegan — Grow Your Glutes (quick tips):**")
    st.markdown("""
    ...
    - **Creatine monohydrate 3–5 g/day** can support higher-quality training and lean mass. Take daily; consistency matters.  
    - Drink plenty of **water** (aim ~2–3+ L/day, more if training hard) and avoid **crash diets**.
    """)
...
def render_daily_intake_guide():
    ...
    with c1:
        daily_kcal = st.number_input("Daily calorie target (kcal)", min_value=1000, max_value=4000, value=1730, step=10)
    ...
    st.info(f"**Per meal:** ~{per_meal_kcal} kcal  |  {carbs_g} g carbs  |  {protein_g} g protein  |  {fat_g} g fat")
...
def render_weight_tracker_tab():
    st.markdown("### Daily Check-ins: Weight • Water • Energy Balance")
    ...
    with st.expander("Add Today's Entry"):
        ...
        with c2:
            ...
            kcal_out = st.number_input("Calories OUT (exercise est.)", min_value=0, max_value=5000, value=500, step=25)
        with c3:
            energy_feel = st.slider("Energy level (1–10)", 1, 10, 7)
        ...
        if st.button("Save Today"):
            ...
            st.session_state.progress_entries.append(entry); save_user_data()
    ...
    if st.session_state.progress_entries:
        ...
        for e in recent:
            ...
            if e.get('photo_path') and os.path.exists(e['photo_path']):
                st.image(e['photo_path'], caption=f"Progress photo — {e['date']}", width=280)

        # Quick tables
        df = pd.DataFrame(sorted(st.session_state.progress_entries, key=lambda x: x['date']))
        st.markdown("#### Trend Tables")
        st.dataframe(df[['date','weight','water_l','kcal_in','kcal_out','kcal_balance','energy_level']], use_container_width=True)
...
def sidebar_controls():
    with st.sidebar:
        ...
        if st.button("Save Progress File"):
            save_user_data(); st.success("Saved!")
        st.markdown("### Media")
        ...
        if os.path.exists(EXERCISE_VIDEOS_DIR):
            vids = [f for f in os.listdir(EXERCISE_VIDEOS_DIR) if f.lower().endswith(('.mp4','.mov','.avi','.webm'))]
        ...
        if st.checkbox("Confirm deletion"):
            st.session_state.completed_exercises=[]; st.session_state.progress_entries=[]
            st.session_state.selected_workout=None; st.session_state.selected_workout_day=None
            if os.path.exists(UPLOAD_ROOT): shutil.rmtree(UPLOAD_ROOT)
            if os.path.exists(USER_DATA_DIR): shutil.rmtree(USER_DATA_DIR)
            ensure_dirs(); st.success("All data cleared."); st.rerun()
# Hourglass Workout Program by Joane Aristilde — PDF-accurate Streamlit app
# Adds: per-exercise video uploads (size-limited), Booty A/B (L2) variation,
# 1-week meal plan selector (Omni / Pescatarian / Vegan), Daily Intake Guide,
# Vegan "grow your glutes" notes incl. creatine & hydration guidance,
...
def main():
    ...
    tab_names = ["🏠 Home", "📖 Progressive Overload", "🎯 Workout Overview", "🏋️ Program", "🍽️ Meal Plan Sample", "📈 Weight Tracker"]
...
# New content pages
def render_home_page():
    st.markdown("## 🏋️‍♀️ Welcome to Your Hourglass Fitness Transformation")

    st.info(
        "Transform your body with this comprehensive 12-week program designed specifically for building your booty, strengthening your core, and sculpting your back and shoulders.")
    ...
    with col1:
        st.write("1.")
    with col2:
        st.markdown("**Read the Workout Overview:** Understand the program structure")

    col1, col2 = st.columns([1, 20])
    with col1:
        st.write("2.")
    with col2:
        st.markdown("**Learn Progressive Overload:** Master the key principle for muscle building")

    col1, col2 = st.columns([1, 20])
    with col1:
        st.write("3.")
    with col2:
        st.markdown("**Choose Your Level:** Start with Level 1 if you're new to this program")

    col1, col2 = st.columns([1, 20])
    with col1:
        st.write("4.")
    with col2:
        st.markdown("**Begin Your Program:** Follow your daily workouts")

    col1, col2 = st.columns([1, 20])
    with col1:
        st.write("5.")
    with col2:
        st.markdown("**Track Your Progress:** Use the weight tracker to monitor changes")

    st.markdown("---")
    st.success(
        "### 💪 Ready to Start?\nNavigate through the tabs below to explore different sections. Remember: consistency and proper form are key to your success!")
    st.markdown("---")
...
def render_progressive_overload():
    ...
    st.warning(
        "**Key Principle:** All exercises with weights should be performed according to the principle of 'Progressive Overload.' This means that you consistently increase your performance.")

    st.markdown(
        "If 12 repetitions feel easy, you raise the weight so that you can only manage 8 repetitions. Over time, you then aim to increase the number of repetitions back to 12. This is how you build muscle.")

    st.markdown("The number of reps listed below may therefore vary as you apply progressive overload.")

    st.markdown("### How It Works:")
    st.markdown("- **Start with proper form:** Master the movement pattern before adding weight")
    st.markdown(
        "- **Increase difficulty gradually:** When 12 reps feel easy, increase weight so you can only do 8 reps with good form")
    st.markdown("- **Build back up:** Work to increase reps back to 12 with the new weight")
    st.markdown("- **Track your progress:** Keep records of weights, sets, and reps")

    st.markdown("### Safety Guidelines:")
    st.markdown("- Never sacrifice form for heavier weight")
    st.markdown("- Increase weight by small increments (2.5-5 lbs typically)")
    st.markdown("- Allow adequate rest between sets and training days")
    st.markdown("- Listen to your body and adjust as needed")
...
def render_workout_overview():
    ...
    st.info(
        "The training plan is structured with different levels and categories to help you progress safely and effectively.")

    st.markdown("### Training Structure:")
    st.markdown("""
    The categories **"Booty"**, **"Legs & Booty"** and **"Light Shoulders & Back"** are divided into Level 1 and Level 2. 

    **Recommended approach:**
    - Start with Level 1 if you're new to this program
    - Progress to Level 2 when you feel confident with Level 1 exercises
    - Beginners should stick with Level 1 until ready
    - The entire split changes when you progress to Level 2
    """)

    st.markdown("### Home Workouts:")
    st.markdown("""
    The **"Abs/Core Only"** category is specifically designed for beginners and can be done at home. 

    If you're motivated on a rest day, you can incorporate "Abs/Core Only" into your weekly plan. However, be cautious not to overexert yourself. The body requires sufficient rest and time for recovery and muscle building.
    """)

    st.success(
        "**Note:** The Level 2 plan is also my current personal workout plan! However, my training plan changes every 12 weeks. The order of exercises is strategically arranged for an optimal training experience and should be adhered to as closely as possible.")